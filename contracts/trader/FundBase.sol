    ...
    Initializable
{
    ...
    function initialize(
        ...
        address perpetual,
        address maintainer,
        bytes32[] calldata initialConfiguration
    )
        external
        initializer
    {
        ...
        _creator = msg.sender;
        initializedCollateral(collataral, collataralDecimals);
        setFeeClaimingPeriod(uint256(initialConfiguration[0]));
        setRedeemingLockdownPeriod(uint256(initialConfiguration[1]));
        setEntranceFeeRate(uint256(initialConfiguration[2]));
        setStreamingFeeRate(uint256(initialConfiguration[3]));
        setPerformanceFeeRate(uint256(initialConfiguration[4]));
    }
